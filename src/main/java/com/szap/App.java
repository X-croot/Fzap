...
public class App {
    ...
    private static final Color themeColor = new Color(76, 175, 80); // #4CAF50
    ...
    private static void createAndShowGUI() {
        JFrame frame = new JFrame("Fzap FTP Brute Force Tool");
        ...
        try {
            Image icon = ImageIO.read(App.class.getResourceAsStream("/fzap_icon.png"));
        ...
        JLabel title = new JLabel("Fzap FTP Brute Force Tool");
        ...
        JTextField ipField = createTextField("127.0.0.1");
        JTextField portField = createTextField("21");
        JTextField threadField = createTextField("10");

        inputPanel.add(createInputLabel("🌐 IP Address")); inputPanel.add(ipField);
        inputPanel.add(createInputLabel("🔌 Port")); inputPanel.add(portField);
        inputPanel.add(createInputLabel("⚙️ Threads (1-99)")); inputPanel.add(threadField);
        ...
        statusArea.setCaretColor(themeColor);
        JScrollPane scrollPane = new JScrollPane(statusArea);
        mainPanel.add(scrollPane);

        statsLabel = new JLabel("");
        statsLabel.setForeground(themeColor);
        ...
        startButton.addActionListener(e -> {
            ...
            for (int i = 0; i < userModel.size(); i++) {
                ...
                for (int j = 0; j < passModel.size(); j++) {
                    ...
                    executor.submit(() -> bruteFTP(ip, port, finalUser, finalPass));
                }
            }
    ...
    private static void bruteFTP(String ip, int port, String user, String pass) {
        ...
        try {
            ...
            if (login) {
                ...
                ftp.disconnect();
                SwingUtilities.invokeLater(() -> {
                    statusArea.append("[✓] Successful FTP login → " + user + ":" + pass + "\n");
                    JOptionPane.showMessageDialog(null, "🔐 FTP Login successful! → " + user + ":" + pass);
                    startButton.setEnabled(true);
                    stopButton.setEnabled(false);
                });
            } else {
    ...
    private static JPanel createListPanel(String title, DefaultListModel<String> model) {
        ...
        panel.setBorder(BorderFactory.createTitledBorder(
                ...
                new Font("JetBrains Mono", Font.BOLD, 14), themeColor));
